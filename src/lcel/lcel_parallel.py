...
promptA = ChatPromptTemplate.from_template(
    "{sentences}について、ポジティブかネガティブか判定してください"
)
promptB = ChatPromptTemplate.from_template(
    "{sentences}について30文字以内で要約してください"
)
...
chain = RunnableParallel(
    ...
    lambda x: (
        f"感情は{x.get('sentiment').sentiment}で、内容は{x.get('summary').summary}"
    )
)
